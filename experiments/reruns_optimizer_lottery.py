...
def parse_args():
    ...
    optimizer_choices = ['Adam', 'AdamW', 'RMSprop', 'SGD']

    default_lr = 1e-3
    default_l2 = 0.0
    default_num_epochs = 100
    default_batch_size = 64
    default_optimizer = optimizer_choices[0]
    default_reruns = 5
    default_workers = 2
    ...
    parser.add_argument('-o', '--optimizer', type=str, choices=optimizer_choices, required=True, help='optimizer')
    parser.add_argument('-reruns', type=int, default=default_reruns, help='reruns')
...
if __name__ == '__main__':
    ...
    for rerun_index in range(args.reruns):
        logger.info('rerun_index: {}'.format(rerun_index))
        ...
        for pruning_index in range(args.steps):
            ...
            # train stats as npz
            acc_stats_path = osp.join(ckpt_dir, 'train_lottery_acc_stats_rerun_{}_{:.3e}.npz'.format(rerun_index, Pm))
            loss_stats_path = osp.join(ckpt_dir, 'train_lottery_loss_stats_rerun_{}_{:.3e}.npz'.format(rerun_index, Pm))
